        ...
        const settings = {
            ...
            server: {
                ...
                data: async (opts) => {
                    ...
                    let countPromise = fetchWithMethod("GET", "/content/count");
...
<template>
    <h1 class="text-3xl">Post List</h1>
    <div class="container my-3">
        <a class="w-1/2 bg-green-400 hover:bg-green-200 hover:text-white border-green400 text-white py-2 px-4 rounded border-0 ml-2"
           href="/post/create">create</a>
        <DeleteAllEmptyDraftButton></DeleteAllEmptyDraftButton>
    </div>
    <div class="container">
        <Grid bind:instance={gridInstance} {...gridSettings}></Grid>
    </div>
    <div class="" on:click={e=>console.log(gridInstance)}>test</div>
</template>
