    ...
    let settings = {
        ...
        sort: {
            ...
            server: {
                url: (prev, columns) => {
                    ...
                    let colName = settings.columns.map(col=>col.id)[col.index];
                    let startChar = prev.indexOf("?") > -1 ? "&" : "?";
                    return `${prev}${startChar}order=${colName}&dir=${dir}`;
                }
            }
        },
        autoWidth:false,
                        ...
                        h('button', {
                            ...
                            onClick: () => {
                                if (confirm("삭제시 복구할 수 없습니다. 삭제하시겠습니까?")) {
                                    deleteDataWithUrl("/content/" + cell, null, true).then(() => {
                                        if (gridInstance) {
                                            gridInstance.updateConfig(()=>settings).forceRender()
                                        }
                                    })
                                }
                            }
        ...
        pagination: {
            ...
            server: {
                url: (prev, page, limit) => {
                    let startChar = prev.indexOf("?") > -1 ? "&" : "?";
                    return `${prev}${startChar}limit=${limit}&offset=${page}`
                }
            }
        },
        server: {
